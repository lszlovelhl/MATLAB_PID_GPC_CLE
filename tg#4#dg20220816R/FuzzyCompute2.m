...
Flow_gas_HHLimit = 30000;        %煤气流量的上限
...
if(abs(et_temp) >= 10)                                       % 调整et 和 ect的权值，使其能精确化控制
    et_weight = 30;
    ect_weight = 1.1;

    Flow_gas_weight = 3000;                             % 温度偏差大于6度时煤气流量调节的速度
    
 else if(abs(et_temp) >= 6&&abs(et_temp)<10)                                       % 调整et 和 ect的权值，使其能精确化控制
    et_weight = 20;
    ect_weight = 0.92;

    Flow_gas_weight = 2200;
else
    et_weight = 12;
    ect_weight = 0.68;
    
    Flow_gas_weight =2000;                           % 温度偏差小于6度时煤气流量的调节速度
