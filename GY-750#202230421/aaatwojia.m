...
b=data(111,:);    %流量滤波值
% b=data(21,:);        %流量实际值（一加热段煤气流量）
c=data(18,17:end);    %阀门设定值（一加热段煤气阀门开度设定值，一加热段空气阀门开度反馈值）
d=data(19,17:end);    %阀门测量值（一加热段煤气阀门开度反馈值，一加热段空气阀门开度反馈值）
figure(1);
subplot(2,1,1)
plot(a,'r');
hold on
...
b=data(166,:);      %一加热段煤烟温度测量值（一加热段煤烟温度）
