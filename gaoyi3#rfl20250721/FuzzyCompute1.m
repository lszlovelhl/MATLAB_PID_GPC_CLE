function [Flow_gas_Compute] = FuzzyCompute1(T_SV,TTT_real,ECT_real_C,Flow_gas_zero_Set,Flow_air_zero_Set,furnacetype)
global fuzzyset default_airgaspotial
% 参数说明
% T_SV 温度设定值  
% TTT_real 温度实际值
...
Flow_gas_HHLimit = fuzzyset(7)*default_airgaspotial(furnacetype);        %煤气流量的上限 更改为 空气流量上限
Flow_gas_LLlimit = fuzzyset(8)*default_airgaspotial(furnacetype);          %煤气流量的下限 更改为 空气流量下限
...
if(abs(et_temp) >= 5)                                       % 调整et 和 ect的权值，使其能精确化控制
    et_weight = fuzzyset(1);
    ect_weight = fuzzyset(11);%1.2;%1.1

    Flow_gas_weight = fuzzyset(2)*default_airgaspotial(furnacetype);                             % 温度偏差大于6度时空气流量调节的速度
    
elseif(abs(et_temp) >= 3&&abs(et_temp)<5)                                       % 调整et 和 ect的权值，使其能精确化控制
    et_weight = fuzzyset(3);
    ect_weight = fuzzyset(10);%0.95;%0.92

    Flow_gas_weight = fuzzyset(4)*default_airgaspotial(furnacetype);
else
    et_weight = fuzzyset(5);
    ect_weight = fuzzyset(9);%0.7;%0.68
    
    Flow_gas_weight =fuzzyset(6)*default_airgaspotial(furnacetype);                           % 温度偏差小于6度时空气流量的调节速度
end
...
%这里更改控制煤气流量为空气流量
Flow_gas_zero = Flow_air_zero_Set;                               % 煤气流量当前值
Flow_gas_min = Flow_gas_zero - 1 * Flow_gas_weight;             % 煤气流量变化最小值                                
Flow_gas_max = Flow_gas_zero + 1 * Flow_gas_weight;             % 煤气流量变化最大值
...
if Flow_gas_Compute > Flow_gas_HHLimit            % 煤气流量位上限 已更改为空气流量上限    
    Flow_gas_Compute = Flow_gas_HHLimit;
end
if Flow_gas_Compute < Flow_gas_LLlimit            % 煤气流量位下限 已更改为空气流量下限
