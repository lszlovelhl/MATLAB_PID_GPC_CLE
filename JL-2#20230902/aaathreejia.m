...
c=data(33,:);    %阀门设定值（二加热段煤气阀门开度设定值，二加热段空气阀门开度反馈值）
d=data(34,:);    %阀门测量值（二加热段煤气阀门开度反馈值，二加热段空气阀门开度反馈值）
f=data(39,:); 
figure(1);
subplot(3,1,1)
% plot(a,'r');
...
b=data(38,:);      %二加热段煤烟温度测量值（二加热段煤烟温度）
...
c=filter2(data(35,:),3); %（data(二加空气流量,:)预热空气流量）
a=data(35,:);% (二加空气流量)
plot(a);
hold on
b=data(117,:);%（二加空气流量滤波）
...
c=filter2(data(36,:),4); %（data(二加煤气流量,:)预热空气流量）
a=data(36,:);%（二加煤气流量）
plot(a);
hold on
b=data(112,:);%（二加煤气流量滤波）
