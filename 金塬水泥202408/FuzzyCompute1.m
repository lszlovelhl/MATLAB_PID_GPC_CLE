...
Flow_gas_HHLimit = fjl_MAX;        %分解炉喂煤的上限
Flow_gas_LLlimit = fjl_MIN;          %分解炉喂煤的下限
...
if(abs(et_temp) >= 6)                                       % 调整et 和 ect的权值，使其能精确化控制
    et_weight = 10;
    ect_weight = 1.5;

    Flow_gas_weight = 0.06;                             % 温度偏差大于6度时煤气流量调节的速度

elseif(abs(et_temp) >= 3&&abs(et_temp)<5)                                       % 调整et 和 ect的权值，使其能精确化控制
    et_weight = 6;
    ect_weight = 1.1;

    Flow_gas_weight = 0.04;
else
    et_weight = 3;
    ect_weight = 0.9;

    Flow_gas_weight =0.02;                           % 温度偏差小于6度时煤气流量的调节速度
...
Flow_gas_Compute = round(Flow_gas_Compute * 100) / 100;
